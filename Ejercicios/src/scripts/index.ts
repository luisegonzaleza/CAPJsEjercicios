...

{
    ...
    class User implements IUser{
        ...
        buscarUsuario(correoUsuario: string){
            let busqueda = directory.filter(correo => correo.email == correoUsuario);
            return this.obtenerUser(busqueda[0].id);
        }

        obtenerUser(id: string){
            const dia = new Date();
            let busquedaU = users.filter(ide => ide.id === id);
            // console.log(busquedaU[0].yearBirth);            
            let anio = (busquedaU[0].yearBirth);
            let anioF:number =+ anio;
            let resultado = {'id':busquedaU[0].id,'name': busquedaU[0].name+' '+busquedaU[0].lastName,'dateBirth': busquedaU[0].dayBirth+'/'+busquedaU[0].monthBirth+'/'+busquedaU[0].yearBirth,'yearsOld': (dia.getFullYear() - anioF)};        
            return resultado;
        }
    }

    let persona1 = new User();
    console.log(persona1.buscarUsuario('francisco.hernandez@axity.com'));
}